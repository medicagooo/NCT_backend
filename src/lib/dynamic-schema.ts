...
function sanitizeQuestionColumnName(value: string): string {
  const normalized = value
    ...
    .replace(/\s+/g, ' ')
    .trim();
  const safe = normalized || '字段';

  if (RESERVED_COLUMNS.has(safe)) {
    return `字段_${safe}`;
  }
